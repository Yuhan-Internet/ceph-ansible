...
options:
    ...
    crush_failure_domain:
        description:
            - Ensure that no two chunks are in a bucket with the same failure domain.
        required: false
    plugin:
        description:
...
def create_profile(module, name, k, m, stripe_unit, crush_device_class, crush_root, crush_failure_domain, plugin, technique, cluster='ceph', force=False, container_image=None):  # noqa: E501
    '''
    Create a profile
    '''
    ...
    if technique:
        args.append('technique={}'.format(technique))
    if force:
        args.append('--force')
...
def run_module():
    ...
    technique = module.params.get('technique')
    ...
    if state == "present":
        ...
        if rc == 0:
            ...
            if current_profile['k'] != k or \
               ...
               current_profile['crush-device-class'] != crush_device_class or \
               current_profile['crush-failure-domain'] != crush_failure_domain or \
               current_profile['crush-root'] != crush_root or \
               current_profile['plugin'] != plugin or \
               current_profile['technique'] != technique:
